...
Ordered-endpoint failover (primary → secondary), signed AUTH, register, heartbeat
(real load for routing), and a request/response serve loop. Reconnects with
backoff forever. stdlib + cryptography only.
"""
...
from eefn.crypto import NodeCrypto
...
class NodeClient:
    ...
    def _parse_endpoint(raw: str) -> tuple:
        ...
        return (host or "127.0.0.1"), int(port or 8081)  # no port -> default 8081
