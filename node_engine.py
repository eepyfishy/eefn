...
Kept self-contained (stdlib + a little urllib for Ollama) so these files can be
copied to a fresh machine verbatim — no eef package needed to run a node. A node
contributes: system.ping, filesystem, launch_application, and model inference
(llm.infer / vlm.analyze) against its local Ollama.
"""
